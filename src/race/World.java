...
public class World extends BaseAppState {
	...
	protected void initialize(Application app) {
		...
		if (type == WorldSpawnType.Infinite) {
			...
			spawnAsObjects(initialState, RaceGLModelFactory.MODEL_WALL, new Vector3f());
			spawnAsObjects(helpArrow, RaceGLModelFactory.MODEL_LINE, new Vector3f());
		}
		
		if (type == WorldSpawnType.Static) {
			spawnAsObjects(staticWorld, RaceGLModelFactory.MODEL_WALL, new Vector3f());
		}
	...
	private void spawn(Piece p) {
		...
		spawnAsObjects(p.walls, RaceGLModelFactory.MODEL_WALL, this.nextSpawn);
	...
	private void spawnAsObjects(Vector3f[] v, String modelType, Vector3f offset) {
		...
		for (int i = 0; i < v.length; i+=2) {
			...
			ed.setComponents(line,
					...
					CollisionShape.Line(v[i+1], modelType.equals(RaceGLModelFactory.MODEL_LINE)),
					new Mass(10000),
